...
function sa_sync_variation_title( $new_title_update_case = array(), $ids = array() )
{
	...
	global $wpdb;
	$case_sql       = 'CASE ' . implode( ' ', $new_title_update_case ) . ' END';
	$ids            = array_map( 'intval', $ids );
	$post_parent_in = implode( ', ', $ids );
	$sql = "
		UPDATE {$wpdb->posts}
		SET post_title = {$case_sql}
		WHERE post_type = %s
		AND post_parent IN ( {$post_parent_in} )
	";
	$wpdb->query(
		$wpdb->prepare( $sql, 'product_variation' ) // phpcs:ignore WordPress.DB.DirectDatabaseQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	);
}
...
function sa_update_product_attribute_lookup_table( $product_ids = array() ) {
	...
	$delete_attribute_data = sa_delete_attribute_lookup_data( $product_ids );
